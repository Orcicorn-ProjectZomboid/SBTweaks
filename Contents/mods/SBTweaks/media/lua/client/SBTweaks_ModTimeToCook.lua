    ...
    if instanceof(item, "Food") then
        ...
        if(item:isCookable() and item:getCookingTime() < item:getMinutesToCook() and item:getHeat() > 1.6) then
            ...
            if player:getPerkLevel(Perks.Cooking) >= 3 then
                ...
                self:drawText("Ready in " .. (math.floor((item:getMinutesToCook()-item:getCookingTime() + 1.5)/2)) .. " Minutes", 40 + 30 + xoff + 200, top + (self.itemHgt - self.fontHgt) / 2, fgText.a, fgText.r, fgText.g, fgText.b, self.font);
            end
        end
    end
end
